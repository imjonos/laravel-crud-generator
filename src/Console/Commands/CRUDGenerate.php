...
class CRUDGenerate extends Command
{
    ...
    protected function import(string $name): void
    {
        ...
        foreach ($this->columns as $key => $column) {
            if(in_array($column['name'], ['created_at', 'updated_at', 'deleted_at', 'remember_token'])) continue;
            if ($n != 0) $columns .= '            ';
            $columns .= '\'' . $column['name'] . '\' => $row[' . $n . ']';
            if ($n != count($this->columns)) $columns .= ',' . PHP_EOL;
            $n++;
        }
    ...
    protected function routes(string $name): void
    {
        ...
        $routes = [
            ...
            "Route::post('" . $route . "/massdestroy', '{$singular}Controller@massDestroy')->name('{$lowerCase}.massdestroy');",
            "Route::put('" . $route . "/{" . $singularLowerCase . "}/toggleboolean', '{$singular}Controller@toggleBoolean')->name('{$lowerCase}.toggleboolean');",
        ];
        if ($this->export) $routes[] = "Route::get('" . $route . "/export', '{$singular}Controller@export')->name('{$lowerCase}.export');";
        if ($this->import) $routes[] = "Route::post('" . $route . "/import', '{$singular}Controller@import')->name('{$lowerCase }.import');";
        ...
        foreach ($this->columns as $column) {
            ...
            if ($column['name'] == 'id' || !in_array($column['name'], $this->systemColumns) && $column['name'] != 'password') {
                if (in_array($column['type'], $eq)) {
                    $scopesTemplate .= $this->makeTemplate(['{{name}}','{{camelName}}'],[$column['name'], ucfirst(Str::camel($column['name']))],'models/scopes/equals');
                } elseif (in_array($column['type'], $date)) {
                    $scopesTemplate .= $this->makeTemplate(['{{name}}','{{camelName}}'],[$column['name'], ucfirst(Str::camel($column['name']))],'models/scopes/from');
                    $scopesTemplate .= $this->makeTemplate(['{{name}}','{{camelName}}'],[$column['name'], ucfirst(Str::camel($column['name']))],'models/scopes/to');
                } else {
                    $scopesTemplate .= $this->makeTemplate(['{{name}}','{{camelName}}'],[$column['name'], ucfirst(Str::camel($column['name']))],'models/scopes/like');
                }
    ...
    protected function languages(string $name): void
    {
        ...
        foreach ($this->columns as $key => $column) {
            $space = "";
            if ($n != 0) $space = "            ";
            $columnsWithTranslations .= $space . "'" . $column['name'] . "' => '" . str_replace('_', ' ', ucfirst($column['name'])) . "'";
            if ($n != (count($this->columns) - 1)) $columnsWithTranslations .= ",\n";
            $n++;
        }
    ...
    protected function controller(string $name): void
    {
        ...
        $this->writeToFile(app_path("Http/Controllers/{$customPath}{$singularName}Controller.php"), $controllerTemplate);
    }
    ...
    protected function request(string $name): void
    {
        ...
        foreach ($this->requestsList AS $request) {
                ...
                foreach ($this->columns as $column) {
                    if (!empty($column['rules'])
                        ...
                        if ($request['rules'] == 'massdestroy') {
                            $rules .= 'selected.*';
                        } else if ($request['rules'] == 'toggleboolean'){
                            $rules .= 'value';
                        } else {
                            $rules .= $column['name'];
                        }
            ...
            $this->writeToFile(app_path("Http/Requests/{$customPath}$name/{$request['name']}Request.php"), $requestTemplate);
        }
    }
        ...
        foreach ($this->viewsList AS $view) {
            if ($view == "create" || $view == "edit") {
                ...
                foreach ($this->columns AS $column) {
                    if (!in_array($column['name'], $this->systemColumns)) {
                        $modelName = "";
                        if ($column['input'] == "select") $modelName = $column['belongsTo']['model'];
            ...
            $this->writeToFile(resource_path("/views/{$customPath}{$this->tableName}/{$view}.blade.php"), $requestTemplate);
        }
    }
    ...
    protected function makeFiltersTemplate($name): string
    {
        ...
        $filters = "";
        foreach ($this->columns AS $column) {
            if (($column['name'] == 'id' || !in_array($column['name'], $this->systemColumns)) && $column['input'] != 'password') {
                $modelName = "";
                if ($column['input'] == "textarea") $column['input'] = "text";
                else
                    if ($column['input'] == "select") $modelName = $column['belongsTo']['model'];
                $postfix = '';
    ...
    protected function getFieldInputType(array $column = []): string
    {
        ...
        if (count($column)) {
            if ($column['name'] == "password") {
                $result = "password";
            } else if (stristr($column['name'], "_id")) {
                $result = "select";
            } else {
            ...
            if (!in_array($column['name'], $this->systemColumns)) {
                ...
                if ($column['name'] !== 'email_verified_at' && $column['name'] !== 'password') {
                    ...
                    if ($column['name'] !== 'password') {
                        ...
                        $fieldsStore .= '$' . strtolower($singularName) . '->' . $column['name'].'."'.($column['type'] == 'string' ? 'newly stored' :'').'"';
                    }
                    if ($column['name'] === 'password'){
    ...
    protected function factory(string $name, array $columns = []): void
    {
        ...
        foreach ($this->columns as $column) {
            ...
            if ($column['input'] === 'select') {
                $return .= '\'' . $column['name'] . '\' => App\\' . $column['belongsTo']['model'] . '::first()->id,' . PHP_EOL . '        ';
            }
